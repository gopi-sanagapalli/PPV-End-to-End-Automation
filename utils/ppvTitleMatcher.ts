...
export function normalizePpvTitle(value: string): string {
  return String(value || '')
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .toLowerCase()
...
export function ppvTitleTokens(value: string): string[] {
  return normalizePpvTitle(value)
    .split(' ')
    .filter(token => token.length > 2 && !STOP_WORDS.has(token));
}

function looksLikeConfigId(value: string): boolean {
  return /[_]/.test(value) || /^ppv[-_]/i.test(value);
}
...
export function isPpvVariant(value: string): boolean {
...
export function isLikelySamePpvTitle(candidateTitle: string, expectedTitle: string): boolean {
  ...
  if (looksLikeConfigId(expectedTitle)) return candidate === expected;
  ...
  if (candidateTokens.length < expectedTokens.length) return false;
  if (isPpvVariant(candidateTitle) && !isPpvVariant(expectedTitle)) return false;

  return expectedTokens.every(expectedToken =>
    candidateTokens.some(candidateToken => tokenMatches(expectedToken, candidateToken))
  );
}

export function scorePpvTitleMatch(candidateText: string, expectedTitle: string): number {
  if (!isLikelySamePpvTitle(candidateText, expectedTitle)) return 0;
  const expected = normalizePpvTitle(expectedTitle);
  const candidate = normalizePpvTitle(candidateText);
  const extraLength = Math.max(0, candidate.length - expected.length);

  if (candidate === expected) return 120;

  const lines = String(candidateText || '')
    .split(/\r?\n/)
    .map(line => normalizePpvTitle(line))
    .filter(Boolean);
  if (lines.some(line => line === expected)) return 110;
  if (lines.some(line => line.startsWith(`${expected} `))) return 100;

  return Math.max(60, 95 - Math.min(35, Math.round(extraLength / 8)));
}
