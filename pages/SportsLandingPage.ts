...
export class SportsLandingPage extends LandingPage {
  ...
  async findPPVContainer(eventData: Record<string, string>, source?: string): Promise<any> {
    ...
    if (src.includes('dont-miss') || src.includes('tile')) {
      ...
      if (!tile) {
        throw new Error('❌ [Home Sport Tile] PPV tile not found in "Don\'t Miss" rail');
      }
    ...
    if (src.includes('upcoming')) {
      ...
      if (env === 'stag') {
        throw new Error(
          `❌ CRITICAL: "Upcoming Fights" tab/section not supported on stag environment for this flow. ` +
          `The home-sport-upcoming flow cannot proceed on stag — use a different source or environment.`
        );
      }
      ...
      if (!tabFound) {
        throw new Error(
          `❌ CRITICAL: "Upcoming Fights" tab not found on ${env} environment within 10s. ` +
          `Cannot proceed with home-sport-upcoming flow.`
        );
      }
      ...
      if (!ppvCard) {
        throw new Error(`❌ [Home Sport Upcoming] PPV card for "${ppvName}" not found under Upcoming Fights tab`);
      }
  ...
  async clickBuyNow(container: any, source?: string): Promise<void> {
    ...
    if (src.includes('tile') || src.includes('dont-miss')) {
      ...
      if (!visible) {
        console.log('🔍 [Home Sport Tile] Searching entire page for Buy now...');
        ...
        if (await pageBuyNow.isVisible({ timeout: 3000 }).catch(() => false)) {
          const box = await pageBuyNow.boundingBox().catch(() => null);
          if (box && box.width > 0 && box.height > 0) {
            console.log(`✅ [Home Sport Tile] Found Buy now on page`);
            await pageBuyNow.click({ force: true, timeout: 8000 });
            console.log(`✅ [Home Sport Tile] Clicked Buy now`);
            await this.page.waitForLoadState('domcontentloaded').catch(() => { });
        ...
        console.log('🔍 [Home Sport Tile] Trying JS-based Buy now click');
        ...
        if (clicked) {
          console.log(`✅ [Home Sport Tile] JS Buy now click executed`);
          await this.page.waitForLoadState('domcontentloaded').catch(() => { });
          const newUrl = this.page.url();
          console.log(`✅ [Home Sport Tile] Navigated to: ${newUrl}`);
          return;
        ...
        throw new Error('❌ [Home Sport Tile] "Buy now" button not found in modal');
      }
