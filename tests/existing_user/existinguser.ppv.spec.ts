          ...
          try {
            ...
            if ((sheetName === 'Home of Boxing' || sheetName === 'Home page') && (isStandalone || onOnboarding)) {
              ...
              try {
                ...
                // Event Date — match month+day from PPV_DATE in container text
                ...
                // Event Description — 2+ significant words from BANNER_DESCRIPTION in container text
                const expectedDesc = eventData.BANNER_DESCRIPTION || '';
                const descWords = expectedDesc.split(/[\s,.:;\-–]+/).map((w: string) => w.toLowerCase()).filter((w: string) => w.length > 3 && !['with', 'from', 'that', 'this', 'then', 'takes', 'their'].includes(w));
